(function(){
    ...
    function getQuote() {
        ...
        .done(function(data) {
            ...
            var newHref = "https://twitter.com/intent/tweet?button_hashtag=QuotesOnDesign&text="+escape(quote.replace(/'/g, "%27") + author);
            $('#quote').html('<i class="fa fa-quote-left" aria-hidden="true"></i> ' + quote +' <i class="fa fa-quote-right" aria-hidden="true"></i>');
            $('.author').html(author);
            $('#twitter-button').attr("href",newHref);
            $('#twitter-button').attr("data-size","large");
